...
function findTwo() {
    const sorted = input.sort((first, second) => first - second)

    let i = 0
    while(i < sorted.length && sorted[i] <= 2020) {
        let j = 0
        while (j < sorted.length && sorted[j] <= 2020) {
            if(sorted[i] + sorted[j] === 2020) {
                return sorted[i] * sorted[j]
            } else if (sorted[i] + sorted[j] > 2020) {
                j = sorted.length
            } else {
                j += 1
            }
...
function findThree() {
    ...
    while (i < sorted.length && sorted[i] <= 2020) {
        ...
        while (j < sorted.length && sorted[j] <= 2020) {
            let z = j + 1
            while (z < sorted.length && sorted[z] <= 2020) {
                if (sorted[i] + sorted[j] + sorted[z] === 2020) {
                    return sorted[i] * sorted[j] * sorted[z]
                } else if (sorted[i] + sorted[j] + sorted[z] > 2020) {
                    z = sorted.length
                } else {
                    z += 1
                }
