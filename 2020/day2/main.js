...
function checkWithRule1(min, max, char, string) {
    let count = 0
    for (let c of string) {
        if (c === char) {
            count += 1
...
function checkWithRule2(indexIs, indexIsNot, char, string) {
    return (string[indexIs - 1] === char && string[indexIsNot - 1] !== char) ||
        (string[indexIs - 1] !== char && string[indexIsNot - 1] === char)
}
...
function main() {
    ...
    for (let item of input) {
        const first = item.split(' ')[0].split('-')[0]
        const second = item.split(' ')[0].split('-')[1]
        const char = item.split(' ')[1].replace(':', '')
        const string = item.split(' ')[2]

        if(checkWithRule1(first, second, char, string)) {
            countRule1 += 1
        }

        if(checkWithRule2(first, second, char, string)) {
            countRule2 += 1
        }
    }

    console.log(countRule1)
    console.log(countRule2)
}
