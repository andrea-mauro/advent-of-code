...
function parseInput(input) {
    const ret = {}
    input.forEach(item => {
        const color = item.split(' bags contain ')[0]
        if (item.split(' bags contain ')[1] === 'no other bags') {
            ret[color] = null
        } else {
            const elements = item.split(' bags contain ')[1].split(',')
            ret[color] = {}
            elements.forEach(element => {
                element = element.trim()
                const num = parseInt(element.split(' ')[0])
                const col = element.substring(element.indexOf(' ') + 1).replace(' bags', '').replace(' bag', '')
                ret[color][col] = num
            })
        }
    })
...
function deepSearchShinyGold(color, rules, visited) {
    const content = rules[color]
    if (visited[color] !== undefined) {
        ...
        for (let key in content) {
            temp = deepSearchShinyGold(key, rules, visited)
            if (temp) {
                visited[color] = true
                return visited[color]
            }
...
function deepCountBags(color, rules, visited) {
    const content = rules[color]
    if (visited[color] !== undefined) {
        return visited[color]
    } else if (!content) {
        ...
        let temp = 0
        for (let key in content) {
            temp += content[key] * deepCountBags(key, rules, visited)
        }
        visited[color] = temp + 1
        return visited[color]
    }
}
...
function main() {
    ...
    for (let color in rules) {
        count += deepSearchShinyGold(color, rules, {}) ? 1 : 0
    }
    console.log(`how many bags contain at least a shiny bag? ${count}`)
    console.log(`how many bags does a shiny gold bag contain? ${deepCountBags('shiny gold', rules, {}) - 1}`)
}
