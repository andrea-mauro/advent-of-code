...
const findWinner = (numbers = [], boards = []) => {
    ...
    for (let ni = 0; ni < numbers.length; ni++) {
        const extracted = numbers[ni]
        for (let bi = 0; bi < boards.length; bi++) {
            const card = boards[bi]

            let board = Math.floor(bi / 25)
            let row = Math.floor((bi % 25) / 5)
            let col = Math.floor(bi % 5)

            if (extracted === card) {

                marked[board] = marked[board] || []
                marked[board].push(card)

                let count = (position[`${board}-r-${row}`] || 0) + 1
                position[`${board}-r-${row}`] = count
                if (count === 5 && winners.indexOf(board) === -1) {
                    winners.push(board)
                    const res = countUnmarked(boards, marked, board) * extracted
                    firstWin = !firstWin ? res : firstWin
                    lastWin = res
                }

                count = (position[`${board}-c-${col}`] || 0) + 1
                position[`${board}-c-${col}`] = count
                if (count === 5 && winners.indexOf(board) === -1) {
                    winners.push(board)
                    const res = countUnmarked(boards, marked, board) * extracted
                    firstWin = !firstWin ? res : firstWin
...
const countUnmarked = (boards = [], marked, boardNumber = 0) => {
    let count = 0
    for (let i = 0; i < 25; i++) {
        const num = boards[i + (boardNumber * 25)]
        if (marked[boardNumber].indexOf(num) === -1) {
            count += num
        }
    }
    return count
...
console.log(findWinner(numbers, boards))
