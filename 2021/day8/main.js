...
const findUnique = (input = []) => {
    ...
    return input.reduce((ret, val) => {
        val.split('|')[1].split(' ').forEach(num => {
...
const findOutput = (line) => {
    const numbers = findNumbers(line)
    return parseInt(line.split('|')[1].split(' ').reduce((ret, val) => {
        const valSorted = [...val].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')
        return `${ret}${numbers[valSorted]}`
    }, ''))
...
const findNumbers = (line) => {
    ...
    const one = firstPart.split(' ').find(number => determineOne(number))
    const four = firstPart.split(' ').find(number => determineFour(number))
    const seven = firstPart.split(' ').find(number => determineSeven(number))
    const eight = firstPart.split(' ').find(number => determineEigth(number))
    const six = firstPart.split(' ').find(number => determineSixGivenOne(number, one))
    const three = firstPart.split(' ').find(number => determineThreeGivenOne(number, one))
    const nine = firstPart.split(' ').find(number => determineNineGivenFour(number, four))
    const zero = firstPart.split(' ').find(number => determineZeroGivenNineAndSix(number, nine, six))
    const five = firstPart.split(' ').find(number => determineFiveGivenSixAndThree(number, six, three))
    const two = firstPart.split(' ').find(number => determineTwoGivenThreeAndFive(number, three, five))

    numberMap[[...one].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '1'
    numberMap[[...two].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '2'
    numberMap[[...three].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '3'
    numberMap[[...four].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '4'
    numberMap[[...five].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '5'
    numberMap[[...six].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '6'
    numberMap[[...seven].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '7'
    numberMap[[...eight].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '8'
    numberMap[[...nine].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '9'
    numberMap[[...zero].sort((a, b) => a < b ? -1 : a > b ? 1 : 0).join('')] = '0'

    return numberMap
}

const determineOne = (one) => {
    return one.length === 2
}

const determineFour = (four) => {
    return four.length === 4
}

const determineSeven = (seven) => {
    return seven.length === 3
}

const determineEigth = (eigth) => {
    return eigth.length === 7
}

const determineSixGivenOne = (six, one) => {
    return six.length === 6 && (six.indexOf(one[0]) === -1 || six.indexOf(one[1]) === -1)
}

const determineThreeGivenOne = (three, one) => {
    return three.length === 5 && three.indexOf(one[0]) !== -1 && three.indexOf(one[1]) !== -1
}

const determineNineGivenFour = (nine, four) => {
    return nine.length === 6 && nine.indexOf(four[0]) !== -1 && nine.indexOf(four[1]) !== -1 && nine.indexOf(four[2]) !== -1 && nine.indexOf(four[3]) !== -1
}

const determineZeroGivenNineAndSix = (zero, six, nine) => {
    return zero.length === 6 && zero !== six && zero !== nine
}

const determineFiveGivenSixAndThree = (five, six, three) => {
    return five.length === 5 &&
        five !== three &&
        six.indexOf(five[0]) !== -1 &&
        six.indexOf(five[1]) !== -1 &&
        six.indexOf(five[2]) !== -1 &&
        six.indexOf(five[3]) !== -1 &&
        six.indexOf(five[4]) !== -1
}

const determineTwoGivenThreeAndFive = (two, three, five) => {
    return two.length === 5 &&
        two !== three &&
        two !== five
}
